...
| `usage_jsonl` | `""` | `SLUICE_USAGE` | `<XDG_STATE_HOME>/sluice/sluice_usage.jsonl` — where the per-call token-usage log is appended, one JSON line per LLM call, which `job-sluice usage` reads back. ON by default, like the triage audit log: the first question anyone asks is about a run that has already finished, and an opt-in log answers it with "no data". Root rather than per-sub-app because triage, cv and track all spend, and what this install cost is one question. No rotation. **Rows for the per-lead stages carry the lead's name, so this file names employers** — like the triage audit log, which records company, role and url. The *report* never prints a lead (it groups by stage and model only), so a pasted report is safe even though the file is not. Not one of the abstain gates above: nothing here gates a lead, it only records what a call cost |
