...
| `slop_allow` | `[]` | phrases from `slop._PHRASES` you legitimately use in your own voice. **Not** abstain-shaped like every other list default above — it *subtracts* from a hardcoded list, so empty means **full enforcement**, the `dossier_allow_hosts` polarity. An entry not found among the stems raises at load, naming the valid ones (the list holds stems like `"leverage"`, not an inflection like `"leveraged"`). A stem that sluice has since RENAMED is migrated automatically with a warning rather than refused, so a lint tweak on our side cannot stop your `cv` commands loading |
