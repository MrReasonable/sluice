...
| `usage_jsonl` | `""` | `SLUICE_USAGE` | **OFF.** Name a file and sluice appends one JSON line per LLM call to it, which `job-sluice usage` reads back; leave it empty and nothing is recorded and no file is created. There is no per-system default location — naming the file *is* how the feature is turned on. Opt-in because **rows for the per-lead stages carry the lead's slug, so the file names the employers you are applying to**, and sluice does not create that uninvited; the same abstain-when-unconfigured posture as the gates above, applied to a write rather than a filter. Deliberately unlike `triage.audit_jsonl` and `sluice_health.json`, which are always on: those record what sluice *decided*, which you need to audit the tool, while this records what you *spent*, which is only useful once you ask. Root rather than per-sub-app because triage, cv and track all spend and what this install cost is one question. No rotation. The *report* never prints a lead (it groups by stage and model only), so a pasted report is safe even though the file is not |
