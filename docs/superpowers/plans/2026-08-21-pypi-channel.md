...
#
# Nothing grafts `docs/`. That tree is gated by NEITHER the neutrality rule (which binds
# `sluice/` and `tests/`) NOR `tests/test_no_leaked_files.py` (which gates paths, not content),
# so publishing it would put 67 unreviewed files on an index that never forgets.
prune tests
```
