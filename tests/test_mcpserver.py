...
def test_cv_run_tool_skipped_selection_for_a_non_shortlist_lead(monkeypatch):
    ...
    (1) `_MinimalCvStore.read_leads` must actually FILTER by `statuses`, mirroring
        `Vault.read_leads` (sluice/core/vault.py::read_leads). A `read_leads` that
    ...
    (2) The note needs a real `.status` attribute, not just `fm["status"]`.
        `out_of_scope_verdict` (core/leads.py::out_of_scope_verdict) reads `n.status`
        directly, in two separate places -- the real `LeadNote` contract's own field
