"""Shared adapter-registry isolation, consumed by every harness-backed tier.

The harness registers its `scripted` fetcher and `recording` renderer through the
real `plugins.register()` API (to exercise the config-key wiring). Those
registrations are process-global and last-wins, so without a teardown they leak
past the test -- re-registering (a WARNING) on every subsequent one, and, once a
second PRODUCTION fetcher/renderer lands and its registry grows an
exact-membership guard (mirroring store/backend), becoming an order hazard.

Both `tests/e2e/` and `tests/functional/` need identical behaviour, so the
fixture lives here (imported and re-exported as autouse by each tier's conftest)
rather than being copied into each -- one implementation, no drift.
"""
import pytest
...
def isolate_plugin_registry():
    """Snapshot the adapter registry before each harness-backed test, restore after.
