...
| A baseline CV at `baseline_rel` (default `My CV/CV.md`) | `cv run` | same, and once for the run rather than once per lead. It never cost a composer call, but under `--lead` it used to surface as a traceback |
